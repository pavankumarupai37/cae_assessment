    ...
    if (!apiResponse.statusMessage) {
      if (apiResponse.status == 200) {
        if (apiResponse.responseJson.length != 0) {
          let groupResult = apiResponse.responseJson.reduce((groupData,item)=>{
          ...
          let combinedData=Object.keys(groupResult).map((title)=>{
            return{
              title,
            data:groupResult[title]
            }
          })
          
          setRosterData(combinedData)
          dispatch({payload:combinedData,type:Constants.ActionTypes.STORE_DATA})
        } else {
        ...
        renderSectionHeader={({ section }) => {
          return (
            <View style={styles.sectionContainer}>
              <Text style={CommonStyles.itemTitle}>{moment(section.title,'DD/MM/YYYY').format('DD MMM. YYYY') }</Text>
            </View>
          )
        }}
